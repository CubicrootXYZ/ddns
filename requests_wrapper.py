    ...
    def __init__(self, *args, **kwargs):
        ...
        for scheme, ConnectionPool in (("http", HTTPConnectionPool), ("https", HTTPSConnectionPool)):
            ...
            for name in AF2NAME.values():
                self.pool_classes_by_scheme["{0}+{1}".format(
                    scheme, name)] = ConnectionPool
                self.key_fn_by_scheme["{0}+{1}".format(
                    scheme, name)] = self.key_fn_by_scheme[scheme]
...
class Session(requests.Session):
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        if family != socket.AF_UNSPEC:
            ...
            url = url._replace(
                scheme="{0}+{1}".format(url.scheme, AF2NAME[int(family)]))
            url = url.geturl()
